...
export default function SolutionVisual({portfolio}) {
  const [currentIndex, setCurrentIndex] = useState(0);

  // AutoPlay every 4s
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentIndex((prev) => (prev + 1) % portfolio.images.length);
    }, 4000);
    return () => clearInterval(interval);
  }, []);

  const getPrevIndex = () => (currentIndex - 1 + portfolio.images.length) % portfolio.images.length;
